...
### 完成内容
- 体重记录页新增 `kg / 斤` 单位切换。
- 体重输入、目标设置、顶部摘要、今日记录、趋势说明、图表 tooltip 和删除确认文案都会跟随当前单位展示。
- Firestore 仍统一保存 kg 数值，不迁移历史数据，不影响仪表盘、BMI 和菜谱营养参考的内部计算。
- 周复盘新增“本周饮食偏好”，从本周饮食描述中即时统计常出现食物和偏向做法。
- 饮食记录页周复盘和独立复盘页周复盘都展示同一份饮食偏好结果。
- 饮食偏好结果暂不持久化，也暂不直接影响菜谱生成算法，避免样本不足时影响菜单稳定性。

### 验证结果
- `npm run lint` 通过 ✅
- `npm run build` 通过 ✅
- 本地启动 `npm run dev -- --port 3001` 后，`/dashboard/weight` 返回 `200 OK` ✅
- 本地启动 `npm run dev -- --port 3001` 后，`/dashboard/food` 返回 `200 OK` ✅
- 本地启动 `npm run dev -- --port 3001` 后，`/dashboard/review` 返回 `200 OK` ✅
- 用户验收通过 ✅
...
### 后续步骤
- 后续可把周复盘饮食偏好作为菜谱生成参考，但需要先观察真实记录样本，避免过早影响菜单生成稳定性。

---

## Step 50: 移动端统计卡片与任务卡片空间压缩 ✅ (完成日期: 2026-04-27)

### 完成内容
- 仪表盘今日进度短卡片在移动端改为 2 列排列，并压缩内边距和字号。
- 仪表盘今日任务四个任务卡在移动端改为 2 列 2 行。
- 仪表盘围度概览的次级围度卡片在移动端改为 2 列排列。
- 体重记录页顶部 4 个摘要卡在移动端改为 2 列排列，并压缩卡片字号和间距。
- 饮食记录页日复盘摘要、日复盘内部短卡片、周复盘摘要在移动端改为 2 列排列；长文本复盘内容保持跨两列展示。
- 运动记录页顶部摘要改为移动端 2 列；运动量记录可能较长，因此移动端单独跨两列展示。
- 独立复盘页今日复盘和本周复盘摘要在移动端改为 2 列排列；长文本内容保持跨两列展示。
- 本次只调整响应式网格、间距和局部字号，不修改数据读取、保存、删除或复盘计算逻辑。
...
### 后续步骤
- 本轮 Step 49-50 已完成；后续按新反馈继续拆分。

---

## Step 49: 体重、饮食、运动今日记录删除 ✅ (完成日期: 2026-04-27)

### 完成内容
- 体重记录页的今日记录列表新增删除按钮和确认弹窗。
- 删除今日体重记录后，会刷新今日记录、体重趋势、今日/最近体重和目标摘要。
- 饮食记录页的今日饮食记录新增逐条删除按钮和确认弹窗。
- 删除今日饮食记录后，会刷新今日记录和日复盘摘要，并让周复盘数据在下次查看时重新加载。
- 运动记录页的今日运动记录新增删除按钮和确认弹窗。
- 删除今日运动记录后，会刷新顶部运动统计和今日运动记录列表。
- 本次只允许删除当天 `today` 下的体重、饮食、运动记录，不新增今天之前历史记录的编辑或删除入口。
...
### Git 提交
- 已提交：`75ef908 feat: allow deleting today's core records`

### 后续步骤
- Step 50: 移动端统计卡片与任务卡片空间压缩（已完成）。

---

## Step 48: 食材到期日期 ✅ (完成日期: 2026-04-27)

### 完成内容
- 食材库存新增真实到期日期口径，新食材保存 `expiryDate`，格式为 `YYYY-MM-DD`。
- 添加和编辑食材表单已从“剩余天数”改为“到期日期”。
- 库存列表基于当前日期展示“已过期、今日到期、N 天后到期、到期 YYYY-MM-DD”。
- 旧食材如果只有 `remainingDays`、没有 `expiryDate`，会显示“需要补充到期日期”，并标记“不参与菜谱”。
- 菜谱生成只使用有 `expiryDate` 且未过期的食材；缺少到期日期或已过期的食材不会进入生成池。
- 食材使用计划也只基于可参与菜谱生成的食材。
- 本次不做旧数据批量迁移，也不根据 `remainingDays` 自动推算真实到期日期。
...
### 后续步骤
- 本轮 Step 46-48 已完成；后续按新反馈继续拆分。

---

## Step 47: 侧边栏导航顺序调整 ✅ (完成日期: 2026-04-27)

### 完成内容
- Dashboard 侧边栏导航顺序已调整为更贴近日常使用路径。
- 新顺序为：首页、食材与菜谱、饮食记录、运动记录、体重记录、围度记录、复盘、个人资料。
- 桌面端侧边栏和移动端抽屉继续共用同一份 `navItems` 配置，因此顺序保持一致。
- 本次只调整导航顺序，不修改路由路径、不修改页面业务逻辑。
...
## Step 46: 轻食手账全局视觉调和 ✅ (完成日期: 2026-04-27)

### 完成内容
- 全局视觉从“炭黑高对比游戏化”调和为“轻食手账 + 轻量任务板”。
- `src/app/globals.css` 调整为更轻的纸白米色方格背景、深橄榄灰文字和柔和鼠尾草绿主色。
- Dashboard 侧边栏从深黑导航改为暖白 / 极浅米色导航，当前页面使用浅绿底和深绿文字高亮。
- 顶部栏改为半透明暖白，头像从强 lime 改为柔和绿色。
- 仪表盘首屏的“今日任务”和“还需变化”去掉黑底，改为浅奶油底、柔和绿色重点卡和暖色进度条。
- Button、Card、Input、Select、Tabs、Checkbox、Dialog、Sheet 等基础组件同步调整为暖白卡片、浅燕麦边框和柔和绿色 focus 状态。
- Dashboard 导航图标从手写 SVG path 改为 `lucide-react` 图标，便于后续继续增加更贴近食物和手账气质的小图标。
...
## Step 44: 全局游戏化视觉基调 ✅ (完成日期: 2026-04-27)

### 完成内容
- 全局背景从冰蓝玻璃感调整为纸白网格底、炭黑主色和少量高饱和状态色。
- Dashboard 侧边栏改为深色导航，当前页面使用 lime 高亮。
- 顶部栏改为白底高对比样式，头像使用 lime 状态色。
- Card、Button、Input、Dialog、Select 等基础 UI 组件统一调整为黑白底、清晰边框和 lime focus 状态。
- 保留已有打印样式，不影响菜谱导出。
- 本次只调整视觉基调和基础组件，不新增功能、不修改数据逻辑。
...
### 后续步骤
- 继续按实际使用体验细化各页面局部视觉层级和移动端细节。

---

## Step 41-43 + Step 45: 账号数据管理与仪表盘任务面板 ✅ (完成日期: 2026-04-27)

### 完成内容
- `memory-bank/modification-plan.md` 和 `memory-bank/implementation-plan.md` 已记录账号数据管理与游戏化视觉改版拆分。
- 个人资料页 `/dashboard/profile` 新增“危险区”，包含“清除历史数据”和“注销账号”两个高风险操作入口。
- 清除历史数据支持删除记录、复盘计划、食材库存、体重目标和菜谱偏好，同时保留账号与基础资料。
- 注销账号流程增加密码重新认证、确认文字、用户数据清理和 Firebase Auth 账号删除。
- Firestore 工具层新增用户历史数据清理、用户数据删除和最近记录存在性查询。
- Auth 工具层新增邮箱密码重新认证和删除当前用户函数。
- 仪表盘 `/dashboard` 新增“今日进度”模块，合并今日/最近体重、初始体重、目标体重和还需变化。
- 仪表盘新增“今日任务”模块，展示称重、饮食、运动、复盘四项任务状态、完成度和连续记录天数。
- 原“快速打卡”和原“进展概览”独立卡片已删除，入口统一收敛到“今日任务”。
- Step 44 全局游戏化视觉基调尚未执行，后续单独处理。

### 验证结果
- `npm run lint` 通过 ✅
- `npm run build` 通过 ✅
- 本地启动 `npm run dev -- --port 3001` 后，用户访问 `/dashboard` 和 `/dashboard/profile` 验收通过 ✅
- 注销账号和清除历史数据属于破坏性操作，本轮完成代码路径、确认弹窗和构建验证，未在生产账号上执行真实删除验证。
...
## Step 40: 轻量营养目标与菜谱份量建议 ✅ (完成日期: 2026-04-26)

### 完成内容
- 食材与菜谱页 `/dashboard/inventory?tab=recipe` 新增“营养参考目标”卡片。
- 基于身高、体重、性别、出生年份和活动水平估算基础代谢、每日消耗和温和减脂热量参考区间。
- 三大营养素按默认 45% 碳水、25% 蛋白质、30% 脂肪拆算为每日克数参考。
- 资料缺失时显示需要补充的字段，并引导前往个人资料页完善。
- 菜谱生成后，每餐展示参考热量、碳水、蛋白质、脂肪和简短份量建议。
- 页面文案明确结果仅供记录和安排份量参考，不作为医疗建议。
- 本次不新增 Firestore 字段，不保存营养目标，按最新资料和体重即时计算。
...
### Git 提交
- 本次提交包含 Step 40 轻量营养目标、菜谱份量建议和对应文档更新。

### 后续步骤
- 新一轮 Step 29-40 产品迭代已执行完毕，可进入整体回归和公开版整理。
...
### 后续步骤
- Step 29-40 已全部完成，可进入整体回归和公开版整理。
