...
- 侧边栏导航共用 `src/app/dashboard/layout.tsx` 中的 `navItems` 配置，桌面端侧边栏和移动端抽屉顺序一致；当前顺序为：首页、食材与菜谱、饮食记录、运动记录、体重记录、围度记录、复盘、个人资料。
- 侧边栏导航已将目标设定合并到体重记录入口，体重记录页同时承担体重记录、趋势查看和目标设置；目标摘要位于页面顶部，目标设置表单默认折叠，通过“设置目标”按钮展开。
- 侧边栏导航已将食材库存和菜谱生成合并为“食材与菜谱”入口，并前置到首页之后；页面内使用 Tab 组织食材库存、一周菜谱和条件设置。
- 侧边栏保留“个人资料”入口，个人资料页用于保存昵称和基础身体资料；未登录用户可只读浏览，写入时仍使用 `AuthRequiredDialog` 拦截。
...
- 体重记录页支持 `kg / 斤` 页面级单位切换；用户输入斤时保存前换算为 kg，读取后按当前单位展示。Firestore 中的 `weight`、`currentWeight`、`targetWeight` 继续统一保存 kg，BMI、营养估算和仪表盘内部计算不受单位切换影响。
- 仪表盘新增“今日任务”，基于今日体重、饮食和运动记录展示称重、饮食、运动、复盘四项任务完成状态、完成度和连续记录天数；原“快速打卡”入口已收敛到任务卡片。
- 移动端短摘要卡片优先使用 2 列紧凑布局：仪表盘今日进度、今日任务、围度概览次级卡片，体重页顶部摘要，饮食页日/周复盘摘要，运动页顶部摘要和复盘页摘要均按该原则压缩；长文本复盘内容继续跨列或单列展示，避免挤压。
- 顶部栏和仪表盘欢迎语优先使用用户昵称，未设置时回退邮箱；右上角昵称和头像区域可点击跳转到 `/dashboard/profile`。
- 首页、仪表盘和体重、围度、饮食、运动记录页共用 `RecordPrincipleNotice`，提示记录用于复盘、请尽量真实填写、历史记录暂不支持修改；体重、围度、饮食、运动的今日误录可通过各页面今日记录列表删除，不扩展任意历史编辑能力。
...
- 周复盘会即时生成饮食偏好结果，包括本周常出现食物、偏向做法和参考建议。该结果来自本周 `foodDescription` 的轻量关键词统计，不新增 Firestore 字段，不持久化结果，当前也不直接驱动菜谱生成。
...
- `deleteDailyRecord` 是体重、围度、饮食、运动今日误录删除的通用函数。
- 当前页面只传入当天 `today` 对应的日期，不提供今天之前历史记录的编辑或删除入口。
- 体重页删除今日记录后刷新今日记录、趋势和目标摘要相关数据。
- 围度页删除今日记录后刷新今日记录、趋势和最近有效摘要。
- 饮食页删除今日记录后刷新今日饮食记录和日复盘摘要，并让周复盘数据在下次查看时重新加载。
- 运动页删除今日记录后刷新顶部统计和今日运动记录列表。
- 未登录用户执行删除动作时由页面级逻辑弹出登录/注册提醒，不向 Firestore 写入删除操作。
...
- 体重页的 `kg / 斤` 切换只影响页面展示和输入解释；保存体重记录和目标设置时会统一换算为 kg 后写入，趋势数据读取后再按当前单位展示。
...
- 偏好分析只读取本周饮食记录中的 `foodDescription`。
- `src/lib/review.ts` 维护轻量关键词列表，用于统计常出现食物和做法。
- 饮食记录页和独立复盘页展示同一份 `weeklyReview.foodPreference`。
- 本功能不新增 Firestore 字段，不保存长期偏好，不直接改写 `recipeSettings`。
- 没有足够匹配结果时显示空状态和继续记录提示。
...
- `expiryDate`：可选，到期日期，格式为 `YYYY-MM-DD`；新添加和编辑食材时使用该字段。
- `remainingDays`：旧字段兼容，旧食材可能仍有该字段；界面不再按该字段推算真实到期日期。
- `userId`
- `createdAt`
...
- 添加、编辑、删除食材后会刷新同页 `ingredients` 状态，食材列表和一周菜谱页库存卡片都会展示数量、单位、可用顿数和到期日期状态。
- 食材到期状态由当前日期和 `expiryDate` 计算，包括已过期、今日到期、临近到期和未到期。
- 旧食材缺少 `expiryDate` 时显示“需要补充到期日期”，不会按旧 `remainingDays` 自动推算日期。
- 生成一周菜谱时直接读取最新库存。
...
- 生成菜谱时只使用有 `expiryDate` 且未过期的食材；缺少到期日期或已过期的食材不会进入生成池。
- 生成菜谱时会使用可用顿数消耗库存：已填写 `servings` 的食材每安排一次扣减 1 顿，不会超过该食材的可用顿数；未填写 `servings` 但有有效 `expiryDate` 的食材继续兼容参与生成，不强制判为不足。
...
- 库存约束：
  ...
  - 所有食材都缺少到期日期或已过期时，不生成菜单，并提示补充真实到期日期。
